...
export const Features = ({ children, color, colorDark, id }: FeaturesProps) => {
  ...
  return (
    <section
      ...
      className={classNames(
        ...
        inView &&
          "is-visible before:opacity-0 sm:before:opacity-100 before:[transform:rotate(180deg)_scale(2)] md:before:[transform:rotate(180deg)_scale(1.05)]",
    ...
    >
      <div className="mt-[4rem] sm:mt-[12.8rem] w-full md:mt-[25.2rem]">
        {children}
      </div>
    </section>
  );
  ...
  return (
    <>
      <div className="relative before:absolute before:inset-0 before:bg-[radial-gradient(ellipse_50%_50%_at_center,rgba(var(--feature-color),0.1),transparent)]">
        ...
        >
          <h2 className="text-gradient pb-2 mb-11 translate-y-[40%] pt-[6rem] sm:pt-[12rem] text-center text-4xl [transition:transform_1000ms_cubic-bezier(0.3,_1.17,_0.55,_0.99)_0s] md:pt-0 md:text-8xl [.is-visible_&]:translate-y-0">
            {title}
          </h2>
...
const FeatureGrid = ({ features }: FeatureGridProps) => {
  return (
    <Container>
      <div className="grid w-full grid-cols-2 place-items-center gap-y-9 gap-x-4 text-sm text-primary-text md:grid-cols-3 md:text-md">
...
const FeatureCards = ({ features }: FeatureCardsProps) => {
  ...
  return (
    <Container>
      <div className="mt-[6rem] md:mt-[16rem] grid w-full grid-cols-1 gap-6 md:grid-cols-2">
        {features.map(({ title, text, image, imageClassName }, index) => (
          <div key={index} title={index.toString()} className="relative">
            <div
              className="relative aspect-[1.1/1] overflow-hidden rounded-[2.4rem] z-20 border border-transparent-white md:rounded-[4.8rem] md:p-14 group"
              onMouseMove={handleMouseMove}
            >
              <motion.div
                className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition duration-300"
                style={{
                  background: useMotionTemplate`radial-gradient(400px circle at ${mouseX}px ${mouseY}px, rgba(var(--feature-color),0.1),transparent 80%)`,
                }}
              />
