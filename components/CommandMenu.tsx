    ...
    <div className={classNames(opened && "opened")} ref={commandMenuRef}>
      ...
      >
        <div className="flex items-center">
          <span className="ml-4 mt-2 bg-white/5 text-white/50 text-xs px-2 leading-10">
        ...
        <div className="flex w-full flex-col text-sm text-off-white">
          {currentOptions.map(({ label, icon: Icon, ...menuItem }, index) => (
            <button
              ...
              className="command-menu-button flex h-[4.6rem] w-full items-center gap-3 px-5 first:bg-white/[0.15] hover:bg-white/[0.05] first:hover:bg-white/[0.15]"
            >
              <Icon />
              {label}
            </button>
